...
class ProcesosAprendiz extends StatefulWidget {
  const ProcesosAprendiz({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ProcesosAprendizState createState() => _ProcesosAprendizState();
}

class _ProcesosAprendizState extends State<ProcesosAprendiz> {
...
// Función que obtiene las solicitudes filtradas según la categoría seleccionada
  Future<List<SolicitudModel>> _getSolicitudesPorCategoria() async {
    final userId = Provider.of<AppState>(context, listen: false)
        .userId; // Obtener el ID del aprendiz logueado
    ...
    // Filtramos las solicitudes relacionadas con el aprendiz actual
    solicitudesFiltradas = todasLasSolicitudes
        .where((solicitud) => solicitud.aprendiz
            .contains(userId)) // Filtrar por ID del aprendiz involucrado
        .toList();
