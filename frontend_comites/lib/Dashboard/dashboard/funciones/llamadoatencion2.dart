...
class LlamadoatencionForm2 extends StatefulWidget {
  const LlamadoatencionForm2({super.key});

  @override
  _LlamadoatencionForm2State createState() => _LlamadoatencionForm2State();
}

class _LlamadoatencionForm2State extends State<LlamadoatencionForm2> {
  int faseActual = 1;
  ...
  late final List<int> selectedAprendicesIds =
      aprendicesSeleccionados.map((aprendiz) => aprendiz.id).toList();
  late final List<String> selectedReglamentosIds =
...
// Métodos auxiliares para simplificar el diseño
  ...
  Future<void> _sendLlamadoatencion() async {
    ...
    // Verificar si algún aprendiz ya está asignado a un llamado de atención
    for (var aprendizId in selectedAprendicesIds) {
      ...
      if (response.statusCode == 200) {
        ...
        final llamadoatencion = aprendizData['llamadoatencionaprendiz'];

        if (llamadoatencion == '1' || llamadoatencion == '2') {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content:
                    Text('El aprendiz ya está en un comité de coordinación.')),
          );
          return;
        }
      } else {
    ...
    // Crear el JSON del llamado de atención
    ...
    print('JSON enviado al backend: $jsonLlamadoatencion');

    // Enviar la solicitud al backend
    final response = await http.post(
      Uri.parse('$sourceApi/api/llamadoAtencion/'),
    ...
    if (response.statusCode == 201) {
      // Lógica de éxito
      print('Llamado de atención enviado correctamente.');

      // Obtener el instructor (usuario logueado)
      final instructor = usuarioLogueado;

      // Crear el modelo de la solicitud a partir de la respuesta del backend
      final llamadoatencionenviado = LlamadoatencionModel(
        ...
        llamadodeatencion: true,
      );

      // Enviar el correo de confirmación al instructor=====

      await enviarCorreoConfirmacionLlamadoatencion(
          llamadoatencionenviado, instructor);

      // Actualizar el campo llamadoatencionaprendiz de los aprendices seleccionados
      for (var aprendizId in selectedAprendicesIds) {
        final updateResponse = await http.patch(
          Uri.parse('http://127.0.0.1:8000/api/UsuarioAprendiz/$aprendizId/'),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({
            'llamadoatencionaprendiz':
                '1', // Asignar 1 o 2 dependiendo del caso
          }),
        );

        if (updateResponse.statusCode != 200) {
          print('Error al actualizar el aprendiz $aprendizId');
        }
      }
