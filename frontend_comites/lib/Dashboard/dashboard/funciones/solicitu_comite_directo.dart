...
class ComiteDirectoForm extends StatefulWidget {
  const ComiteDirectoForm({super.key});

  @override
  _ComiteDirectoFormState createState() => _ComiteDirectoFormState();
}

class _ComiteDirectoFormState extends State<ComiteDirectoForm> {
  int faseActual = 1;
  ...
  Future<void> _fetchReglamentos() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Filtrar los reglamentos con gravedad "muy grave"
        List<dynamic> filteredData = data
            .where((item) =>
                item['gravedad'] != null &&
                item['gravedad'].toString().toLowerCase() == 'muy grave')
            .toList();

        setState(() {
          // Convertir los datos filtrados a objetos ReglamentoModel
          reglamentos = filteredData
              .map((item) => ReglamentoModel.fromJson(item))
              .toList();
        });
      } else {
      ...
      print('Error al cargar reglamentos: $e');
                    ...
                    ElevatedButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => const MainSolicitud()),
                        );
                      },
                      style: ElevatedButton.styleFrom(
                      ...
                      child: const Text('Comite Normal'),
                    ),
                  ],
                ),
              ),
