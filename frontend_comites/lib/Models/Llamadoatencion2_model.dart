...
List<LlamadoatencionModel2> llamadoatencion2 = [];

/// Clase que representa una solicitud en la aplicación.
class LlamadoatencionModel2 {
  final int id;
  ...
  bool llamadodeatencion;

  LlamadoatencionModel2({
    required this.id,
...
factory LlamadoatencionModel2.fromJson(Map<String, dynamic> json) {
  return LlamadoatencionModel2(
    id: json['id'] ?? 0,
    aprendiz: json['aprendiz'] ?? [],
    fechallamadoatencion: json['fechasolicitud'] != null
...
Future<List<LlamadoatencionModel2>> getLlamadoatecionByUser(int userId) async {
  ...
  if (response.statusCode == 200) {
    ...
    return data
        .map((json) => LlamadoatencionModel2.fromJson(json))
...
Future<List<LlamadoatencionModel2>> getLlamadoatencion2() async {
  ...
  String url = "$sourceApi/api/llamadoAtenciond/";
  ...
  if (response.statusCode == 200) {
    ...
    for (var solicitudData in decodedData) {
      llamadoatencion2.add(
        LlamadoatencionModel2.fromJson(solicitudData),
      );
    }
    // Devolver la lista de solicitudes
    return llamadoatencion2;
  } else {
...
Future<LlamadoatencionModel2> _getLllamadoatencionDetails(
  ...
  if (response.statusCode == 200) {
    ...
    return LlamadoatencionModel2.fromJson(data);
  } else {
